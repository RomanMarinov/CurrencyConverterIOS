...
**Тап по уведомлению** (если напоминание включено в Настройках):

- **Свёрнутое приложение** — iOS выводит приложение на передний план, курсы ЦБ автоматически обновляются, внизу показывается баннер «Курсы обновлены».
- **Закрытое приложение** — приложение запускается, после старта выполняется то же обновление курсов и показ баннера.
- **Открытое приложение** — при тапе по баннеру уведомления курсы также обновляются без дополнительных действий.

Обновление инициирует `NotificationCoordinator`: тап фиксируется как отложенное действие, затем `RatesStore.refresh()` вызывается, когда UI готов.
